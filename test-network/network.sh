...
function createOrgs() {
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/IoTFeds/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating AUTH Identities"

    # createAUTH

    # infoln "Creating UNS Identities"

    # createUNS

    # infoln "Creating HCS Identities"

    # createHCS

    # infoln "Creating UNITOV Identities"

    # createUNITOV

    # infoln "Creating UOA Identities"

    # createUOA

    # infoln "Creating GOC Identities"

    # createGOC

    # infoln "Creating DISBA Identities"

    # createDISBA

    # infoln "Creating IDIBAPS Identities"

    # createIDIBAPS

    # infoln "Creating VIS Identities"

    # createVIS

     infoln "Creating IoTFeds Identities"
  ...
  infoln "Generating CCP files for IoTFeds"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/IoTFeds/msp organizations/fabric-ca/IoTFeds/tls-cert.pem organizations/fabric-ca/IoTFeds/ca-cert.pem organizations/fabric-ca/IoTFeds/IssuerPublicKey organizations/fabric-ca/IoTFeds/IssuerRevocationPublicKey organizations/fabric-ca/IoTFeds/fabric-ca-server.db'
