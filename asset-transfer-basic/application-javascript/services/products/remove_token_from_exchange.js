...
const removeTokenFromExchange = async(req, res, next) => {
    ...
    try {
        ...
        console.log('\n--> Submit Transaction: RemoveTokenFromExchange, lets user remove a token from exchange list');
        // let result = await contract.submitTransaction('BuyProduct', product_id, seller, buyer, access_times, access_period);
        let result = await contract.submitTransaction('RemoveTokenFromExchange', product_id, user_id);
    ...
    //}
			...
			console.log('Removing token from exchange failed with error: '+error);

			res.status(403).send('Removing token from exchange failed with error: '+error)

    }

}
module.exports = removeTokenFromExchange;
