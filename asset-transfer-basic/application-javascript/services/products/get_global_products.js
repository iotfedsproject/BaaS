...
const getGlobalProducts = async(req, res, next) => {
    ...
    try {
        ...
        console.log('\n--> Submit Transaction: GetGlobalProducts, gets all the products in the global marketplace');
        // let result = await contract.submitTransaction('CheckAccess', product_id, tx_id);
        let result = await contract.evaluateTransaction('GetGlobalProducts');
    ...
    //}
        ...
        console.log(`Get global products products failed with error: `+err);

        res.status(403).send({error: 'Get global products failed ...'+err})
        // .responses[0].response.message
    }

}
module.exports = getGlobalProducts;
