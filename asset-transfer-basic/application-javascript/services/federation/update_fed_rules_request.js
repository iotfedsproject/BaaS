...
const updateFedRulesRequest = async(req, res, next) => {

    const fed_id = req.body.fed_id;
    const requestor_id = req.body.requestor_id;
    // the user to remove
    const new_rules = req.body.new_rules;
    ...
    try {
        ...
        console.log('\n--> Submit Transaction: updateFedRulesRequest, requests to change rules of a federation');
        result = await contract.submitTransaction('UpdateFedRulesRequest', requestor_id, fed_id, JSON.stringify(new_rules));
        ...
        for (let i=0; i<votersMailArray.length; i++){
            ...
            sendmail(votersMailArray[i], tokens[i]);
				...
				// sendmail(votersMailArray, tokens);
...
module.exports = updateFedRulesRequest;
