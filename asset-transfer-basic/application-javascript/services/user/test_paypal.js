...
const testPaypal = async(req, res, next) => {
   // console.log("KOITA DW:",req)
    let kati=req.body.data.transaction;
    console.log("KOITA DW1:",JSON.stringify(kati.history[0].data.payload.amount));
    console.log("KOITA DW2:",JSON.stringify(kati.history[0].metadata));
    let deposit=kati.history[0].data.payload.amount;
    ...
    await contract.submitTransaction('UpdateUserBalance', user, deposit);
    console.log('*** Result: committed');

    const contract2 = network.getContract(chaincodeName2);
    let newUserID= await getClientID(user);
    //let minterID= await getClientID('iotFedsAdmin');
    console.log('\n--> Submit Transaction: Transfer, Transfer tokens');
    let result2 = await contract2.submitTransaction('Transfer', newUserID, deposit);
